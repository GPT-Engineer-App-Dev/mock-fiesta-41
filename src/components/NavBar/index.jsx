...
export default function NavBar() {
  ...
  return (
    <Popover className={` ${addBlur && "drop-shadow-lg backdrop-blur-md"} fixed top-0 w-full z-[100] transition-all duration-300`}>
      <motion.div viewport={{ once: true }} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ type: "linear", duration: 0.5 }} className="container">
        <div className="flex justify-end items-center px-0 py-4 gap-4 sm:px-6 lg:py-8 lg:gap-10">
          <div className="mr-auto">
            <a href="#home">{darkTheme ? <img className="h-12 w-auto" src={images.brandLogoDark} alt="brand icon" height={12} width={245} /> : <img className="h-12 w-auto" src={images.brandLogoLight} alt="brand icon" height={12} width={245} />}</a>
          </div>
          ...
          <motion.div
            ...
            onClick={handleMode}
          >
            {darkTheme ? <SunIcon className="h-6 w-6 cursor-pointer opacity-100" /> : <MoonIcon className="h-6 w-6 cursor-pointer opacity-100" />}
          </motion.div>
        </div>
              ...
              <div className="pt-5 pb-6 px-5">
                <div className="flex items-center justify-between">
                  <motion.div
                    ...
                    onClick={handleMode}
                  >
                    {darkTheme ? <SunIcon className="h-6 w-6 cursor-pointer opacity-100 text-white" /> : <MoonIcon className="h-6 w-6 cursor-pointer opacity-100 text-white" />}
                  </motion.div>
                  <div className="-mr-2">
                  ...
                  <nav className="grid grid-cols-1 gap-7">
                    {navlinks.map((navlink) => (
                      <a key={navlink.name} href={navlink.href} className="text-base font-medium text-white w-max relative">
                        {navlink.name}
                        {navlink.type === "new" && <span className="block absolute right-0 text-xs bg-green-400 text-primary py-1 px-2 rounded-md font-bold">NEW</span>}
                        {navlink.type === "wip" && <span className="block absolute top-0 -right-12 text-xs bg-yellow-400 text-primary py-1 px-2 rounded-md font-bold">WIP</span>}
                      </a>
                    ))}
                  </nav>
